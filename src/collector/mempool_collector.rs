...
// Transaction collector for mempool.

pub struct MempoolFullCollector {
    provider: Arc<dyn Provider<PubSubFrontend>>,
}

impl MempoolFullCollector {
...
#[async_trait]
impl Collector<Transaction> for MempoolFullCollector {
    fn name(&self) -> &str {
        "MempoolTxCollector"
    }
    ...
    async fn get_event_stream(&self) -> eyre::Result<CollectorStream<'_, Transaction>> {
        let stream = self
            ...
            .await
            .wrap_err("fail to subscribe to pending transaction(full) stream")?
            .into_stream();
...
impl Collector<Transaction> for MempoolCollector {
    ...
    async fn get_event_stream(&self) -> eyre::Result<CollectorStream<'_, Transaction>> {
        let stream = self
            ...
            .await
            .wrap_err("fail to subscribe to pending transaction(hashes) stream")?
            .into_stream();
